...
Web 前端会在每次进入新路径时跟踪该页面通过统一 API 客户端发出的首批请求，并在这些请求全部成功、失败或超时结束后一次性显示页面。进度条中的完成数表示已经结束的本页请求数；页内筛选、标签切换、行情轮询和后台刷新不会重新遮挡已有内容。若进度长时间不结束，应检查浏览器 Network 中仍处于 pending 的接口以及后端日志，而不是反复刷新页面。首次组件分包异常会先自动恢复一次，只有恢复仍失败才显示手动重试入口。
