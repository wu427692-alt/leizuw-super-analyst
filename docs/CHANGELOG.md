...
## 2026-09-01 — 概念题材分页、研究闭环与首屏性能

- 将概念题材工作台拆为“研究总览、题材轮动、题材库、个股映射、交易验证”五个页内页面，避免全部内容同时加载和纵向堆叠。
- 各页面按需请求数据，切换时保留查询状态；题材库和个股映射采用独立分页。
- 新增“数据证据 → 题材强弱 → 成分归因 → 个股暴露 → 计划验证”研究链路，以及证据、题材、个股、风险四道验证门；只输出研究状态，不输出买卖指令。
- canonical 题材总览由全量 ORM 大字段读取改为轻量 ID/名称扫描后按页补全，并增加 30 秒隔离副本缓存，降低重复打开与多人访问延迟。
- 新增缓存隔离与跨检索复用回归测试；题材服务 24 项测试通过，前端 lint 与生产构建通过。生产实测关键词检索由约 6.9 秒降至约 0.24 秒。

- [体验] 概念题材工作台从单一长页面拆为研究总览、题材轮动、题材库、个股映射和交易验证五个页内分页；当前分页写入 URL，可刷新、分享和继续研究，不再需要在超长信息流中反复滚动。
- [性能] 概念题材页面改为按当前分页按需取数和渲染：总览不再同时请求轮动、生命周期、机构语料、自选暴露、成分变化、股票雷达与题材详情；页面激活后的静默刷新也只更新当前工作台，题材与成分每页分别收敛到 24 条。
- [研究闭环] 新增固定研究闭环轨道和交易研究验证台，把数据截止日、题材阶段、多源个股归因和自选集中度组成四道证据门；候选只能进入“补证据 / 继续跟踪 / 进入量化验证”，不直接生成买卖指令，并可携带股票和题材进入量化回测。
- [性能] 巨潮定期报告正文优先使用宽松许可 PDFium 在独立限时进程中完整逐页提取，`pypdf` 仅作限时兼容降级；缓存继续绑定 PDF SHA-256，并新增页数完整性、引擎版本、耗时和失败原因审计，超时或部分结果不再冒充已读取正文。
- [修复] Kimi 长报告改用独立 300 秒超时和 2 次总尝试，不影响普通机构段子；首轮综合研判输出上限收敛到 6,000 token，行情、估值和事实账本采用保留首尾的确定性抽样，证据包限制数量与摘要长度，同时为定期报告、研报 PDF、录音转写和网页正文保留来源席位。长请求发出前明确提示可能需要数分钟，后续只按真实返回和已完成章节推进进度。
- [改进] 公司研究可从 Tushare 已解析的官网地址创建任务级精确域名候选；普通 HTML 过短时，仅该公司官网可读取首个同源 `type=module` 资产，且只接受 `application/javascript`、`text/javascript`、`application/ecmascript` 或 `text/ecmascript`。提取器不执行代码，只保留足量静态自然语言，优先使用完整中文文本并排除 CSS/路径/模板占位；证据保留 HTML 与 JS 的组合哈希、资产链接和独立哈希。
- [口径修复] 窗口 Alpha 改用与简单日收益回归一致的算术复利公式，避免对大截距股票使用对数收益近似造成高估；样本、R² 和置信度仍完整保留。
- [数据迁移] 窗口 Alpha 口径升级加入低锁增量迁移，启动与周期任务自动校正既有结果并记录版本，避免同页新旧口径混用。
- [轮动历史] 题材快照新增 20 个交易日的真实历史增量回补，仅使用支持历史日期的同花顺与东方财富日度接口；旧日期只进快照账本，不覆盖当前目录。
- [指标口径] 题材轮动只有在凑齐 5 个有效交易日后才显示 5 日动量，历史回补尚未完成时保持空值，不用稀疏样本冒充完整窗口。
- [交易日] 题材历史回补改用交易所开市日历；日历不可用时的本地降级也会过滤周末，避免非交易日空请求和无限重试。
- [轮动口径] 5 日题材动量改为逐日收益复利累计，不再简单相加百分比；可用日期计数不会超过用户选择的研究窗口。
- [可视化] 题材详情新增 20 日真实多源走势带，展示复合累计收益、最近 5 日单日涨跌与独立来源数，直接连接轮动判断和成分归因。
- [本体] 扩充分层规则覆盖明确的 AI、先进电子、智能汽车、消费、周期材料、基建、环保和金融长尾题材，六源目录语义覆盖率由 85.5% 提升至 89.3%；模糊节点仍保留原名不强行归类。
- [自动化] 新增自选股时立即唤醒概念题材补齐队列并置顶处理，与行情、公告和半年历史回填同时异步启动，不再等待六小时常规复核或排在旧自选之后。
- [研究] 概念题材新增市场轮动 × 机构语料生命周期交叉台，仅在题材名精确一致或明确产业簇一致时分期并公开双方原始题材；宽泛家族不拼接，阶段是描述性规则而非交易信号。
- [透明度] 概念题材页新增真实更新节奏台，分别公开供应商目录、全市场成分、自选股复核和页面读库频率；页面自动刷新不再被表述为上游题材秒级变化。
- [体验] 概念题材页新增吸顶研究导航，市场轮动、机构语料、自选暴露、成分变化、股票雷达和题材宇宙可一键跳转；手机端使用横向滚动导航减少长页面反复滑动。
- [性能] 个股题材透镜默认响应裁剪为高权重前 40 个长尾题材，完整全量仍可 CSV 导出；题材/股票详情增加短时请求复用，再次查看同一标的直接展示最近成功结果并后台更新。
- [导出] 个股题材画像与当前用户自选题材暴露新增 CSV 导出，包含层级、来源共识、权重、Beta/Alpha、置信、截止日和集中度口径；自选数据继续按登录用户隔离。
- [数据质量] 新增题材成分变化账本，追踪供应商在既有题材中的后续新增/退出股票，并排除首次建库基线；无可确认变化时保持空白，不再把历史回填伪装成市场新事件。
- [研究] 自选股题材暴露新增集中度体检，显示主导共同题材覆盖率、多源归因覆盖、共同题材数和每股独立主线均值；计算按股票数量等权并明确不代表持仓市值权重。
- [个性化] 概念题材页新增当前用户自选股题材暴露图，按每只股票自己的最新归因日聚合多源业务主线，展示主导题材、独立主线数、标签重叠率及多人持有题材集中度；账户之间严格隔离。
- [图谱] 题材关系新增高度重叠、同主题簇、同题材家族和跨题材共现分类，并公开当前题材独占成分数，避免把同义标签与跨行业共同成分混读。
- [口径修复] 申万行业不再全部折叠成“申万/市场行业体系”，罗马数字层级后缀只用于层级说明，通信设备等真实行业名称继续参与自选股题材暴露与关系分析。
- [图谱] 题材详情新增真实成分关系图，按共同股票数、当前题材覆盖率与 Jaccard 重合度排序相邻题材，并公开对方题材规模；关系由有效成分事实计算，不靠名称相似度生成。
- [体验] 机构语料候选点击后打开证据抽屉，直接展示统一股票身份、提及次数、原始机构段子与供应商目录；仅已确认主题可继续进入市场成分归因，未确认候选不再跳进空搜索结果。
- [研究] 新兴题材发现分改为非饱和组合，综合对数语料规模、近期集中度、相对前窗加速度、证据质量与独立供应商确认；无历史基线的新主题显示 `NEW`，不伪造夸张增速。
- [AI语料] 概念题材页新增“机构语料新兴题材雷达”，聚合近 30 日机构段子的主题加速度、情绪结构与明确股票提及，并区分语料候选、单源待交叉和多源已确认；AI 候选不自动进入市场共识、成分或归因权重。
- [研究] 概念题材跨股票共识改按独立语义簇排名，加入相近标签重叠率与主导题材占比；CPO、光模块、光通信等重复命名不再被当作三条独立逻辑，原始来源节点仍完整保留可审计。
- [数据] 六源题材长尾新增区域、国资、指数风格、机构持仓、业绩财务、治理资本、宏观跨境和供需价格等结构层，真实目录语义分层覆盖率提升至 85.5%；非业务结构标签不参与股票产业主线排名。
...
- [改进] 市场总览首页移除不再具备决策价值的北向资金展示，并回收指标栏空间。
- [改进] 公共首页重构为白底橙色瑞士编辑风格，移除导致大段空白的滚动隐藏与科技光效，新增八个真实界面、研究流程、数据口径和移动端紧凑布局。

- [改进] 使用手册重构为白底橙色任务型文档站，加入功能搜索、分类导航、真实页面截图编号标注、跨页面研究流程和可读性优化，并修正首屏标题孤字换行。

- [新功能] 行业与公司调研支持一键下载正式 Word 与 PDF 报告，统一封面、章节、表格、图表、页码和资料来源版式。
- [改进] 行业与公司调研报告页改为纯报告阅读模式，移除任务状态、模型参数、证据哈希和 QA 控制台信息，并将内部方括号证据编号改为上标来源序号。

- [改进] 全站浅色模式重塑为橙白金融终端：以 `#FF4F00` 统一导航、页签、主按钮、焦点和重点数据轨道，白色/浅灰承担页面与卡片层级；首页、机构段子、量化、概念题材、数据获取、使用手册、注册登录和管理员界面共用同一视觉语法，行情红涨绿跌及成功、警告、错误色继续保持独立语义。
- [改进] 完整使用手册改为高对比白色编辑风：正文与说明字号提升到可读尺度，同时压缩首屏、章节、步骤卡和表格留白；8 张真实产品截图新增 36 个按操作点定位的局部放大视图，保留整页编号、局部截图、操作解释和全屏核验四层阅读方式。
- [改进] 首页改为紧凑型首屏决策台：主行情使用紧凑图表，新闻速递前移并与主行情同屏展示，重要新闻获得更大的摘要区；市场证据、涨跌分布、海外市场、分析入口和自选股行情同步压缩重复留白，减少纵向滚动。
- [改进] 首页“最新情报”改为仅展示财经新闻源的“新闻速递”，并按现有新闻评分口径自动置顶重要度不低于 70 的重大新闻，以及重要度不低于 60 且命中当前用户自选股的新闻；股评、机构段子和公司资料不再混入新闻列表。
- [文档] 补齐行业/公司调研的巨潮总时限、请求预算、单分类分页上限和分类间隔配置说明，并明确预算耗尽与分页截断会保留不完整状态。
- [数据] 行业调研从多源概念成分中按权重、独立来源与置信度固定选择 3～5 家代表企业，限量直采 Tushare 公司基础、财务指标、收入利润和经营现金流；同一报告期核心收入/归母净利润不足三家会触发受限报告，关键数不由 AI 生成。
- [可视化] 行业报告新增代表企业共同报告期的收入、利润/经营现金流及 ROE/毛利率对照图，同业事实进入证据编号、事实账本、模型输入和快照哈希。
- [新功能] “行业调研”升级为“行业与公司调研”：支持行业/上市公司双模式，名称或代码解析后直接打通 Tushare 公司资料、财务三表、财务指标、业绩预告、一致预期、历史行情，以及巨潮年报、半年报、一季报、三季报 PDF 链接。
- [数据] 研究任务启动时唤醒知识星球 MCP 增量同步和两年研报库，并统一召回机构段子、文件、图片、新闻、企业事实与互联网原链接；每一来源独立记录成功、部分、缺失或失败状态。
- [录音] 对课题严格匹配的录音先调用阿里云语音识别转成文字，再将录音纪要与转写摘录作为独立待核验证据交给 Kimi；宽泛同帖录音不连带入选，转写失败不会阻断其他证据成稿。
- [AI研究] 行业与公司报告固定优先使用 Kimi，分别采用八章专业框架，默认正文目标 20,000 字；首轮结论先发布，长报告在后台并行撰写，所有重要判断只能引用固定快照中的真实证据编号。
- [可视化] 报告阅读器新增标准封面、真实数据源生产线、证据结构/时间/质量图、公司财务与行情图、机构原图画廊、章节目录和反证区；支持 Markdown、完整证据 JSON 下载以及打印为 PDF。
- [改进] 行业与公司调研升级为 Evidence-to-Decision Research OS：新增研究契约、按章来源计划、10 阶段 AI 流程和确定性数据质量门，关键源缺失时只生成可查看、可下载且明确列出缺口的受限报告，不再用长正文冒充完整结论。
- [改进] 公司研究扩展 Tushare 估值筹码、技术因子、股东人数、管理层、质押、解禁、回购、增减持、分红、业绩快报、机构调研、两融、北向和龙虎榜数据；各接口独立记录覆盖与权限失败，单一高级接口不可用不阻断其余证据。
- [改进] 巨潮年报、半年报和季度报除元数据与 PDF 链接外，新增有限数量的正文下载和重点章节解析；链接数与已读正文数分开统计，可通过 `INDUSTRY_RESEARCH_FILING_MAX_DOCS` 控制单任务解析上限。
- [文档] 行业与公司调研文档新增权威方法依据、行业/个股数据矩阵、证据分层、质量和发布门、`limited` 语义、已知限制与后续补强路线。
- [研报正文] 从两年本地研报索引按课题相关度与日期选择有限数量的高价值 PDF，经过 HTTPS 域名白名单、流式大小/页数限制、哈希缓存和单份失败隔离后抽取正文；链接、摘要与真正已读正文分开计数。
- [网页正文] 行业/公司调研从搜索结果中按权威度与相关度选择少量 HTTPS 白名单页面，经过公网 DNS、同来源族重定向、HTML MIME、超时和流式限额校验后提取正文并保存原文/正文哈希；搜索有链接但没有成功正文时报告明确受限。
- [公告] 公司调研改为召回巨潮全部 26 类公告元数据，覆盖日常经营、治理、股权、融资和风险提示；只有定期报告进入有限 PDF 正文解析，避免不受控下载。
- [财务口径] 同报告期三表和财务指标按最新实际/正式公告日、修订标志与合并口径选取，保留版本账本并排除旧版，避免依赖接口返回顺序混用修订数据。
- [研究治理] 新增指标—值—单位—期间—证据编号事实卡、完整模型输入哈希、截止日未来数据隔离、按渠道时效评分、后台任务租约与旧 worker 写入保护。
- [发布门] Kimi 默认成为行业/公司报告硬要求；AI 未配置、录音命中但未转写、研报正文缺失、独立总编建议受限、伪引用或跨章冲突未解决时，都只能生成明确的有限报告。
- [体验] 行业与公司调研页新增六层“数据—用途—能力边界”方法矩阵和研报 PDF 正文实际读入量，十步 AI 流程增加独立总编与反方审查状态。
- [改进] 新增 DeepSeek V4 Flash / Kimi Code 日历路由：每月 9—23 日优先 Kimi、24 日至次月 8 日优先 DeepSeek，日期边界无需重启，失败时仅在两个低成本通道间回退；机构段子、日报、录音纪要、一站式取数和通用分析统一记入用量账本。
- [修复] 每日机构段子报告成功后不再因迟到分析改变源哈希而被五分钟轮询重复生成；全量统计保留，代表性证据默认压缩至 48 条并可配置，人工强制重跑语义不变。
- [新功能] 新增“概念题材查看”一级工作台：保留同花顺、东方财富、开盘啦、通达信和申万原始口径，并组织为题材家族、产业链簇、来源题材、成分股和公司证据五层结构；支持题材/股票/来源检索、真实分页、20/60/120 日归因和手机端浏览。
- [研究] 建立可解释的个股题材权重与双因子归因：权重由来源共识、业务证据、市场热度和题材专属性组成；题材 Beta 使用剔除目标股票的成分组合，并控制沪深300，单列 R²、样本数、置信等级、窗口 Alpha 和公司独特证据，避免把单源标签或残差误写成市场共识。
- [数据] 概念题材后台自动同步每日校准六源目录、优先补自选股反向归属并按游标渐进补齐全市场成分；目录覆盖与成分覆盖分开显示，单源失败后续跑且不重复覆盖已入库事实。
- [修复] 概念题材动态筛选键保留 AI、CPO 等原始大小写，解决家族点击后召回为空；全市场成分扫描升级为持久化任务账本，服务器重启后从未扫描或最久未扫描节点继续。
- [研究] 题材 Beta 增加标准误、t 统计量和 95% 置信区间，置信等级同时约束样本数、来源数、R² 与统计显著性。
- [数据] 题材本体规则修正“电子竞技/电商/电子烟/电子身份证/飞行汽车”等歧义词，并统一 CPO、光模块、eVTOL、AI PC、氢能、钠电池、核电和 HJT 等市场别名；服务启动时只重算规范归属，不改写任何来源事实。
- [研究] 题材专属性同时考虑题材成分稀缺度与公司题材聚焦度；股票透镜增加四项权重拆解，并把单一来源、高专属性题材降级为“非共识线索”，与多源市场共识严格分区。
- [修复] 20/60/120 日股票透镜按各自窗口读取最新归因日期，不再跨周期串用快照；无有效样本时不再显示伪造的 `0.00~0.00` Beta 区间。
- [数据] 完整成分刷新会把已退出题材的股票安全标记为失效并保留历史记录；疑似截断或空响应不会触发清退，避免上游异常误删成分关系。
- [体验] 总搜索框输入股票名称或代码后新增“股票画像直达”，直接打开该股全部题材共识、权重、Beta/Alpha 与公司证据，不再要求用户先猜题材再翻成分列表。
- [研究] 业务证据分打通本地机构段子：最近一年语料必须同时命中规范题材标签并明确提及成分股才可加分；股票透镜公开供应商理由分、本地语料分、篇数与样例证据。
- [新功能] 增加多源题材轮动轨道与每日来源快照：同一规范题材按来源日涨跌中位数聚合，展示轮动分、独立来源数、5日累计和真实历史轨迹；首日只显示 `NEW`，不反推虚假历史。
- [研究] 股票透镜“主要题材”严格要求至少两个独立来源；高专属性单源标签只进入待核验 Alpha 线索区，并在没有多源题材时明确说明。
- [数据] 六源题材本体扩展为农业食品、基础设施与交通物流、生态环保等 18 个家族，5,559 个原始节点中 4,288 个进入明确语义层级；页面单列 77.1% 语义覆盖率，剩余长尾保留原名而不强行误分类。
- [性能] 题材家族与产业链筛选改为共享语义索引和数据库内分页，目录更新时自动失效；不再在每次点击时把 5,559 个节点全部载入内存重分组，真实库总览与家族切换稳定在约 25–55 毫秒。
- [数据] 六源目录逐项公开交易日、扫描覆盖率、节点规模与来源可靠度；全市场成分补库采用按源公平配额，防止同花顺大目录长期占满批次而使东方财富、开盘啦、通达信和申万覆盖停滞。
- [研究] 新增题材对照台，可任选两个题材比较成分 Jaccard 重合、多源确认、归因覆盖、共同成分和各自高权重独占股票；当前题材支持全量 CSV 导出，带权重、Beta/Alpha、R²、样本、置信和来源理由。
- [可视化] 题材详情新增“核心成分 × 六来源”共识矩阵，逐股显示每家来源是否纳入；来源共识从单一计数下钻为可核验的原始投票结构，并支持手机横向浏览。
- [可视化] 题材详情新增 Beta/Alpha 四象限气泡图，横轴为题材敏感度、纵轴为窗口 Alpha、气泡大小为题材权重，直接区分高弹性跟随、独立强势和落后成分；无有效样本不参与绘制。
- [研究] 新增题材研究优先队列，将成分分为多源核心、高 Beta 弹性、低 Beta 正 Alpha 和高 Beta 负 Alpha 背离四组；点击个股继续进入证据透镜，明确定位为核验顺序而非自动交易信号。
- [体验] 每张题材卡新增规范题材独立来源数/原始节点数，首屏详情优先打开三源以上且已有成分的题材，避免单一供应商的高热标签被误当成默认市场共识。
- [研究] 总览增加 2源+/3源+/4源+ 市场共识门槛，服务端先按规范题材的独立来源数筛选再分页；实测 4源以上仍可召回 1,209 个来源节点，并保留每一源原始口径。
- [自动化] 概念题材页移除手动后台更新按钮，总览和轮动在页面激活及固定周期内并行静默刷新；单次上游波动继续展示最近成功快照，不再弹出阻断式失败提示。
- [文档] 站内使用手册由七个扩展为八个工作台，新增概念题材真实运行截图和从共识门槛、六源矩阵、Beta/Alpha 到个股证据与 CSV 导出的操作说明。
- [个性化] 概念题材成分与当前用户自选股打通：命中标的显示星标并支持一键筛选，服务端按登录用户隔离计算，避免不同用户自选池互相污染。
- [研究] 新增跨题材股票雷达，在同一归因截止日按多题材共识、高 Beta、独立正 Alpha 和高专属性四种口径发现优先研究标的；单源题材不会混入市场共识榜，结果支持直接下钻个股证据。
- [分层] 二级题材簇支持真实成分聚合：点击光通信产业链等主题即可查看其全部源节点、规范题材、去重股票、来源覆盖和代表暴露，避免“分层按钮只会过滤、看不到对应股票”。
- [移动端] 概念题材宇宙在手机端改为 12 条真实分页，轮动、跨题材雷达和二级聚合保持横向卡片浏览；390px 视口验证无横向溢出，并把默认页面高度和初次渲染量约减半。
- [AI语料] 题材详情新增机构语料共识层，严格按 AI 主题字段召回近 90 日机构段子，展示多空分布、置信加权观点分、14 日热度变化与原文证据；样本上限和“观点不等于事实”口径同时可见。
- [信息架构] 题材宇宙默认使用规范题材去重视图，同名跨来源节点合并成一张卡且成分数按全部来源去重计算；保留“全部源节点”切换，既降低重复噪声又不牺牲来源审计。
- [稳定性] 概念题材未知统计在首次加载时显示为 `—` 而非误导性的零；二级题材聚合加入页面激活静默刷新，单次上游波动继续保留最近成功结果。
- [口径修复] 六套题材目录与五个独立数据提供方分开计量：东方财富板块/题材库保留两套原始节点，但共识只计一票；后台增量修复存量归因的提供方数量、共识分和最终题材权重。
- [研究] 个股题材透镜新增 20/60/120 日跨周期归因剖面，分别展示 Beta、窗口 Alpha、样本和置信；新增稳定 Beta 与双窗口正 Alpha 计数，不把单一窗口结果泛化为长期属性。
- [数据质量] 概念题材首屏公开目录交易日、同日目录数、成分扫描覆盖与归因截止日；对目录滞后、归因落后和失败节点分别告警，不再以单一更新时间掩盖数据层差异。
- [后台任务] 新增低负载跨周期归因补算：优先自选股与跨题材高权重标的，分批补齐主线题材的 20/120 日窗口，离开页面后继续运行且不阻塞前台查询。
- [修复] 跨周期补算统一自选股短代码与交易所代码，并跳过没有多源主线的候选继续执行后续标的，解决单个不合格自选股占住整轮任务的问题。
- [研究] 公司独特 Alpha 线索新增业绩、订单、产品、治理、风险和供需分类及措辞方向统计；相同标题跨渠道自动去重，并明确语义方向不是股价预测或因果结论。

- [文档] 新增网站内“使用手册”：使用七个现行工作台的真实截图，按注册登录、数据口径、逐功能操作、推荐研究流程、数据来源和常见问题组织图文说明；截图支持点击放大，公开首页、登录后桌面导航及手机“更多”菜单均可进入。手册页面公开只读，应用、API 和用户数据继续强制登录。

- [官网] 公开首页由概念宣传改为可核验的真实产品导览：新增市场总览、自选股、机构段子与录音、投资情报、量化、一站式取数和行业调研七个现行页面实机截图；逐项写明用途、使用数据与输出，删除虚构指标和无法验证的营销表述，并补齐桌面、手机、延迟加载和减少动态效果适配。介绍页截图使用独立只读静态路径公开展示，内部页面、API 与用户数据仍需登录。

- [界面] 自选股超级看板移除面向运维的渠道状态、历史回填表、重复进度条和手动补齐入口，历史数据继续由后台自动维护，主页面集中展示行情、事实、情报与分析。
- [改进] 登录后按用户意图与后台空闲时段预热高频页面数据，并为首页、量化、行业调研和一站式取数增加跨页面请求复用，减少重复查询和切页等待。
- [改进] 云端 nginx 直接发送前端哈希资源并拦截常见无效扫描请求，降低 Python 服务瞬时 CPU 与公网切页延迟。

- [改进] 全站切页改为即时展示页面骨架与最近成功数据，不再用全屏闸门等待接口；桌面悬停、键盘聚焦和手机触摸导航会提前加载目标页面代码，首页、自选股、情报、机构段子洞察和行业蓝图增加跨路由请求合并与短时陈旧可用缓存，后台继续自动刷新且账号切换时清空个性化缓存。

- [设计] 全站完成五轮研究终端视觉升级：第一轮统一中性灰、研究蓝、字号、间距与组件层级；第二轮重构首页与自选股事实轨道；第三轮提升机构段子、录音检索与热力图可读性；第四轮统一量化、行业调研、投资情报与一站式取数工作台；第五轮补齐手机端密度、键盘焦点、减少动态效果和加载状态。新增“证据轨道”持续展示来源、时间、新鲜度与覆盖率，减少跨页面口径误读。

- [运维] 面向阿里云 2 核 4 GB 实例完成十轮无功能裁剪优化：建立资源基线、轻量启动预热、SQLite 精简字段读取、有界洞察缓存、数据源退避与集中内存回收、前端可见性轮询、用户后台任务隔离、生产日志回收、全量回归和健康守卫发布；启动预热稳定在约 1.7 秒，Web 常驻内存约 670 MB。
- [修复] 分时行情查询在长假或上游连续中断超过本地自然日窗口时，改为相对最新已入库交易日读取最近 1/5 个交易日，避免数据库有历史行情但页面空白；管理员录音转写配置重新统一为标准字段协议，避免配置接口因类型不一致返回 500。

- [修复] 前台访问权限改为强制有效用户会话，取消仅凭可信 IP 自动登录并废止旧版自动登录 Cookie；服务端同步拦截内部页面直达，API 文档改为仅管理员访问，云部署固定启用用户与管理员鉴权，避免共享公网 IP 或配置漂移绕过注册审批。

- [改进] 官网首页右上角新增“注册”入口，访客点击后直接打开申请注册标签；手机端隐藏次要管理员按钮，为注册与进入研究终端保留稳定空间。

- [修复] 云端以网页可访问为最高优先级：提高 Web 容器内存与 OOM 保护，压低独立采集容器 CPU/内存和系统 OOM 优先级；新增每 30 秒健康守护、失败三次自动重启、4 GB swap 峰值兜底，以及由宿主机直接提供的自动重试恢复页。

- [修复] 阿里云部署将全渠道投资情报轮询从 Web 进程拆到独立采集容器，限制各自 CPU/内存并在每个数据源完成后主动回收大型对象；采集任务异常或内存重启不再中断网页访问，Web 端也不会因页面刷新误启第二套采集循环。

- [修复] 录音纪要兼容 DeepSeek V4 Flash 的推理输出：结构化提取默认关闭深度思考，空正文、截断或临时上游错误会自动重试，前端不再显示模型原始响应；AI 阶段失败后保留并复用已完成逐字稿，不再重复下载录音或调用阿里云转写。

- [迭代] 公司录音研究连续完成四版升级：后台任务持久化并在服务重启后自动续跑，失败任务支持原地重试；阿里云转写支持金融热词和预期说话人数；页面内纪要升级为研判、证据链、带时间戳逐字稿三联工作台；完成的录音小作文自动写入统一小作文检索库和投资情报事件库。

- [新功能] 公司录音 AI 纪要接入阿里云百炼异步文件转写，支持真实任务状态、句级时间戳和说话人分离，并继续由 DeepSeek 生成可下载纪要。

- [新功能] 机构段子与录音支持勾选公司录音后提交用户独立的后台 AI 纪要任务：按需临时下载、语音转写、DeepSeek 分段提取与合并研判，页面内查看录音小作文，并下载 Word、Markdown、结构化 JSON 或含逐录音转写文本的完整 ZIP；源音频转写后立即删除。

- [改进] 用户端移除持仓、AI 建议、独立回测和告警入口，旧链接统一返回市场首页，后台数据与接口保留以避免历史数据丢失。

- [新功能] 新增“行业调研”独立页面与极速后台课题，统一召回两年研报、机构段子/录音、公告、行情财务、企业事实和新闻评论，立即生成证据蓝图，并在后台持续补强产业链地图、公司候选、证据库、访谈问题与可追溯 AI 报告。
- [体验] “行业调研”新增置顶任务中心，按运行中、已完成、失败筛选并实时显示后台进度；已完成任务可恢复证据快照并自动定位到研究报告，启动课题会复用已生成的证据缓存，避免重复扫描。
- [研究] 行业调研升级为不少于 2 万字的后台 AI 长篇任务：固定全渠道证据快照后并行完成八个研究章节，逐章显示进度，正文保留 evidence_id 引用并附证据目录；任务列表只传输轻量摘要，打开报告时再按需读取全文。
- [修复] 行业调研任务刚创建、证据快照尚为空时不再把空对象当成完整数据读取，消除启动 AI 任务后偶发的页面加载失败。
- [修复] 行业调研新输入与旧任务彻底隔离：新行业请求会抢占并丢弃过期蓝图响应，任务列表不再自动覆盖当前课题，旧行业的快照、进度和报告不会混入新行业；页面刚打开时也可立即输入并生成蓝图或启动后台报告。
- [改进] 行业调研改为“答案优先”：已有同主题报告自动恢复并在任务历史之前展示一句话判断、趋势、龙头、核心瓶颈和持续跟踪指标；回车或主按钮直接启动研究，“资料覆盖”降为辅助入口。AI 首轮结构化结论生成后立即持久化并可阅读，八章长报告继续在后台并行补写。

- [改进] 公开介绍页重构为蓝色 Aurora 滚动叙事体验：首屏使用动态证据网络与研究终端视觉，中段以数据脉冲串联事实汇聚、证据整理、研究验证和持续跟踪，下段更新市场总览、自选股、机构段子与录音、投资情报、量化回测和一站式取数六大真实能力；补齐滚动揭示、视差、减少动态效果与手机端适配。
...
### 概念题材查看迭代 69：Alpha 证据站内追溯

- 公司独特 Alpha 线索统一进入站内情报证据页，避免外链打断研究上下文；原始来源仍由证据页继续承载。
### 概念题材查看迭代 70：对照台加入行情与覆盖率

- 双题材对照不再只比较成分数量：新增最新日涨跌、真实历史累计收益、多源确认率和归因覆盖率，帮助区分“涨得像”与“成分/证据真的相似”。
### 概念题材查看迭代 71：数据成熟度筛选

- 增加“已有成分股 / 已完成归因 / 多源且已归因”筛选，让用户既能查看完整题材目录，也能一键收敛到当前真正可研究的数据集。
### 概念题材查看迭代 72：长尾题材本体补全

- 补齐光通信协议/器件、半导体材料、医药事件、资源品、区域政策、资本事件等高热长尾题材的语义归类，优先处理多源或高热但此前落入“其他”的真实目录节点。
### 概念题材查看迭代 73：产业链二级层级扩容

- 新增光互连、AI芯片、通信网络、数据安全、先进封装、显示电子、工业母机、动力电池、医疗器械、氢能核聚变、零售电商、资源品、交通基建等二级链条，提升“分层概念题材”的可导航性。
### 概念题材查看迭代 74：归因覆盖口径透明化

- “归因结果”不再只显示可能跨窗口重复的行数；新增当前60日窗口覆盖的题材数、股票数和多源可研究题材数，直接揭示底层可用范围。
### 概念题材查看迭代 75：双题材离线对照包

- 题材对照台支持导出CSV，逐股列出双方是否纳入、题材权重、独立来源数、Beta、窗口Alpha及共同/独占关系，便于离线复核和组合研究。
### 概念题材查看迭代 76：可复现研究深链

- 题材详情可复制包含题材ID和20/60/120日归因窗口的研究链接；重新打开后直接恢复对应题材与窗口，不再只能回到总览重新查找。
### 概念题材查看迭代 77：生产口径最终对齐

- 已归因/可研究筛选与总览覆盖统计统一限定当前60日窗口；方法版本同步到 v1.77，避免筛选、顶部指标与方法说明引用不同窗口。
### 概念题材查看迭代 78：研究链接兼容降级

- Safari、隐私模式或剪贴板权限受限时不再静默失败，也不弹错误框；系统改用浏览器兼容复制，仍失败时把题材ID与归因窗口写入地址栏供直接复制。
