...
- 小作文研判台信息流新增“导出 Excel”，按当前全库搜索和筛选条件导出全部命中结果、AI 分析标签与完整原文；超长原文会在独立工作表中无损分段。
...
- 全局页面容器改为移动端紧凑边距，输入框避免 iOS 自动缩放，抽屉在窄屏减少无效留白。
- 首页指数、自选股和周期切换改为可横滑轨道；K 线周期控件、海外行情、最新情报和 AI 入口按手机宽度重排。
- 自选股列表改为横向股票轨道，避免新增自选股后把主行情推到首屏之外；详情栏目使用吸顶导航。
- 小作文筛选、主题热力迁移、原文阅读和分页提升手机触控尺寸与文字可读性。
- 量化运行历史在手机端由宽表转换为逐条研究卡片，同时压缩研究流程与方法卡片的纵向占用。
- 问股工作区按动态视口和手机上下导航重新计算高度，消息、操作按钮和输入区适配窄屏。
...
- [改进] 手机端固定底部导航调整为“首页、自选、机构段子、数据下载、更多”，将高频查看与资料获取入口直接放到底栏，问股和投资情报台保留在“更多”导航中。

- [修复] 统一首页与自选股看板的行情交易日口径：收盘后自动补采遗漏的收盘快照，较新交易日的部分实时快照不再被旧日线覆盖，并明确标注未到 15:00 的最新时点；用户自选股尚未回填当日日线时，使用同日 15:00 后交易所快照补齐分时收盘点，卡片、主报价、昨收与图表不再出现两套结果。

- [新功能] “小作文雷达”更名为“机构段子与录音”，“信息流”更名为“检索与获取”；勾选录音后改为持久化后台下载与 ZIP 打包，显示逐文件、字节与传输进度，离开页面后任务继续运行并可在 48 小时内返回下载。

- [新功能] 量化回测支持按用户隔离的持久化后台任务：离开页面后继续执行，服务重启后恢复排队，完成后可在运行历史中打开不可变结果快照。
- [重构] 量化首页升级为任务中心，按“假设—数据—约束—执行—稳健性”展示研究生命周期；用户端隐藏“后台全量机构预计算”等系统维护运行，只展示本人主动创建的研究任务。
- [改进] 量化结果新增四段证据链、任务定义、样本外超额、95% 置信区间、最大回撤、有效样本与交易转译，明确给出信号触发、模拟入场/退出、仓位上限、停止采用条件和下一步验证。

- [维护] 清理已被现行路由取代的旧投资情报桌面页、无业务调用的旧分析状态仓库和零引用前端依赖；修正 FAQ 中失效的 `--market-only` 参数。后端总门禁现在自动兼容项目虚拟环境、`python` / `python3`，并支持用 `PYTHON_BIN` 显式指定解释器；前端依赖在保持现有主版本的前提下升级到无已知 `npm audit` 漏洞的版本；测试标记、辅助客户端收集和 SQLite 迁移不再产生项目自身可消除的弃用/收集警告。
- [修复] 巨潮公告改为自选股强保证链路：不再依赖可能截断的全市场最近页命中自选股，而是每 60 秒逐股精确增量查询最近 3 天；新增自选股和服务启动会自动把最近 365 天按 30 天分片回补，SQLite 持久化已完成分片、失败次数和重试时间，网络失败后从缺口续跑且公告 ID 幂等去重。新增只读状态接口展示逐股覆盖进度，并纳入同步看门狗自动拉起。
- [改进] 全站路由首屏门改为只聚合最初 750ms 内发出的核心请求：请求正常完成时约 180ms 后展示，单个慢接口最多遮挡 1.4 秒，之后由页面自身骨架继续加载；解决一个非关键接口重试时整页隐藏数秒的问题。
- [改进] 实时股票/指数报价增加进程内同请求合并与 1 秒微缓存，后台标签页停止仪表盘轮询；小作文信息流继续 15 秒增量刷新，重型洞察/趋势/报告按 30～60 秒复用准备结果，减少重复 SQLite 聚合和公网并发。
- [改进] 服务端增加已签名用户会话短缓存、API JSON gzip 与 SQLite 16MiB 连接缓存；多连接桌面运行默认关闭 mmap，避免重复常驻映射，账号停用立即清理会话缓存；macOS 启动强制使用 LiteLLM 随包模型目录，移除启动时可选 GitHub 请求。
- [改进] 启动预热新增 `essential/full` 档位并默认使用核心档：不再开机串行全算洞察图谱、超级看板和关键词重建，逐任务记录耗时；生产日志基线中原 32～38 秒重型预热被移出默认启动关键期。
- [新功能] 前台增加姓名与密码注册、管理员审批、密码登录和可信 IP 自动识别；管理员在 `/admin/access` 查看脱敏网络信息，可批准、拒绝或停用账号，同一 IP 对应多个账号时强制密码登录避免串号。
- [改进] 自选股成员关系、问股会话、分析历史与任务、持仓账户、告警规则、AI 决策信号、自然语言量化规则/运行记录及传统回测结果改为服务端按用户隔离；行情、公告、研报和小作文等事实数据库继续共享采集，避免为每个用户重复抓取同一份市场数据。
- [修复] “量化回测与数据利用”的五个研究方法不再只是同一事件研究的不同按钮：事件研究、多因子选股、情报与趋势共振、机构胜率追踪和信号组合分别拥有独立数据说明、参数模板、样本构建算法与专属结果区；后端持久化方法级入选口径和诊断指标，前端可通过方法 URL 恢复对应工作台。交互回测改用后台自动同步的本地行情并采用独立 180 秒预算，栏目切换与30秒状态刷新不再重载后台基线、覆盖刚完成的方法结果。
- [新功能] 新增沉浸式公开简介首屏：根地址以可交互的全渠道证据流展示实时行情、全渠道情报、小作文洞察与量化回测能力，点击“进入财经情报台”后进入 `/app`；原应用导航统一回到应用首页，并补齐桌面、手机与减少动态效果的响应式体验。
- [修复] macOS 热更新改为 PID 单实例启动：LaunchAgent 卸载后会确认旧 Uvicorn 进程真正退出再复制运行目录和重启，避免旧服务占用 8000 端口而新服务反复预热；量化机构基线延后 120 秒并以低 CPU 优先级执行，启动阶段优先保障首页、问股和交互回测。
- [修复] 全站可用性巡检修复测试数据污染生产缓存、A股休市后快照冒充下一交易日、旧实时快照覆盖最新 Tushare 日线以及指数卡片与主 K 线日期不一致；首页与行情库现在统一使用交易所时间和最近已完成交易日，异常休市时段数据不再参与报价、涨跌幅或零轴，收盘后秒级报价接口自动回落到当日正式日线收盘及上一交易日零轴。
- [改进] 小作文进度、洞察看板和超级自选聚合新增数据库版本感知缓存及开机预热；行情轮询和不可见标的预热改为后台请求，不再阻塞页面首屏进度门。量化事件读取改为只查询研究所需字段，真实 5,288 事件回测由 52.6 秒降至 4.9 秒，并修复自然语言把“小作文”等语料通称误当关键词导致零样本。
- [修复] AI 建议自动解析模型遗留的 JSON 字符串列表为可读文本，并归档生产库中的历史测试信号；自选股增删与行情 worker 解耦，配置已成功落盘时不再因后台 worker 临时重启失败向用户返回 500，后续由同步看门狗恢复。
- [修复] 量化行情同步由“每轮最多 60 只逐股补取”改为按最近 5 个已完成交易日调用 Tushare 全市场横截面并只落库量化标的，约 15 分钟自动校准一次；新增目标交易日、逐标的最新日、当前/滞后/无行情数量和覆盖率，禁止用单只股票的全局最大日期冒充全量最新。自然语言关键词改为多词匹配并剔除重复方向词，五张结果图补齐最小尺寸约束。
- [测试] 真实全量机构基线完成 8,142 个事件、6,077 个到期样本和 1,798 个股票代码验证；量化服务、规则增删改、事件回测、DeepSeek 规划与白名单执行、预计算、六个前端栏目逐项通过。
- [新功能] Web 应用拆分为公开前台和独立管理员控制面：行情、情报、小作文、回测和数据分析保持免登录访问；`/admin` 单独登录后管理 API/模型、数据源、同步任务、用量审计与系统设置。服务端从“全 API 登录”改为敏感接口清单鉴权，密钥、配置和后台任务控制即使被前端误暴露也会返回 401；公开页面收到局部 401 不再被强制跳转登录。
- [修复] 小作文 AI 不再把“信息不足，未形成有效摘要”等占位语保存为成功：模型必须依据标题/正文/附件形态返回具体摘要，批量响应中的缺项或占位项会立即降为单篇重试，仍失败则进入持久化退避队列；服务启动会自动扫描并回炉最多 5,000 条既有占位摘要，无需人工按钮。
- [改进] 小作文链路改为全自动：服务启动即开启知识星球 MCP 10 秒游标增量、只为新增或正文变化的纪要创建 AI 任务、每日自动生成前一日报告，并由同步看门狗在失败或心跳超时时自动恢复；页面每 15 秒静默刷新且回到前台立即更新，移除手动刷新、同步启停、失败重试、日报生成和历史抓取按钮，后台刷新失败时保留已有数据而不弹出阻断提示。
- [改进] 小作文“洞察图谱”新增短期14日、中期90日、长期1年和自定义起止日期的统一研究窗口，主题热力按日/周/月自动聚合；移除信息量有限的情绪脉冲与催化/风险末端，改为复用本地 Tushare 日线，对同股同日小作文去重后计算事件后1/5/20日收益、沪深300超额收益、胜率、95%置信区间和0/1/3/5日领先相关，并支持按需 DeepSeek 解释可复核统计，明确禁止把相关性写成因果。
- [改进] 小作文洞察图谱的“主题热力迁移”改用每日主题提及占比与相对当日峰值计算集中度，并按当前真实分布显示五档亮度及图例，解决全局最大值压暗多数格子的问题；新增可追溯主题规范层，将 CPO、NPO、LPO、光模块、硅光、光互联等强同义词/明确子主题合并为“光通信”，单篇去重计数、原始标签完整保留，点击规范主题检索时自动扩展全部同义词。
- [改进] 原有问股、持仓、AI 建议、传统回测与告警完成五轮数据升级：新增单股全渠道共享工作台，将实时行情、Tushare 估值/财务/技术/筹码/机构/股东、巨潮公告、知识星球小作文、天眼查和真实股评统一为带时间与来源的本地事实底稿；问股自动注入压缩证据上下文，四个旧页面复用同一接口，采用短时共享缓存和静默降级，单个上游异常不再阻断原功能。
- [改进] 自选股“一致预期研究工作台”升级为 20+5 去重样本：分别读取最多 20 篇相关小作文与 5 篇股票标签中仅含当前标的的单股专属小作文；DeepSeek 按观点提出时间、预测对应期间、验证窗口和失效条件提取预期，界面重构为全宽机构研究终端，新增预测时间线、证据矩阵、双样本原文栏与页内原文查看。
- [改进] 自选股超级看板新增全市场股票名称联想，支持输入中文名称、拼音简称或股票代码；选择候选后使用规范市场代码加入自选并自动触发半年数据回填。
- [新功能] “量化回测与数据利用”重构为六区研究工作台：新增五步研究管线、策略工坊、自然语言回测、结果实验室、真实数据资产地图和不可变运行历史；DeepSeek 只生成受约束任务和服务器模板代码，用户确认后由白名单引擎执行，不开放任意代码、Shell、SQL、密钥或下单权限。
- [改进] 小作文量化默认只使用 AI 已完成且通过阈值的观点，原始未分析语料降为显式探索层；新增同股同机构重复信号聚类、固定交易成本、95% 自助法置信区间、t 值、盈亏比、收益分布、月度队列与成本敏感性，修复关键词展开导致样本数虚高且结果缺少稳健性证据的问题。
- [改进] 量化页面统一为首页同系工业研究终端，高对比线性图标替代不可见图标；首屏等待真实方法、资产、运行与后台状态全部返回后一次性展示，局部接口失败只显示内联状态，不弹出阻断窗口。

- [新功能] 小作文雷达新增“洞察图谱”独立页：基于已分析原文的真实共现关系，串联知识星球来源、主题、个股和催化/风险，并增加 14 日信息脉冲、主题热力迁移、个股讨论动量、多空分歧、跨模型共识、待核验队列和证据链完整度；所有节点均可回到全库原文检索。
- [改进] 自选股“股评监控”停止获取和展示雪球讨论热度，只保留东方财富股吧真实公开帖子；帖子点击后在当前页面详情抽屉展示作者、正文摘录、时间、互动数和图片，外部原帖仅作为可选入口。
- [新功能] 新增服务端同步自修复看门狗：统一监测投资情报、知识星球 MCP 与盘中行情 worker 的线程和同步心跳；发现 worker 退出、心跳超时或来源到期未同步时自动重启/唤醒，并通过状态 API 展示修复记录；失败来源增加受控重试间隔，休市时不误触发行情补抓。
- [修复] 全站入口语言从错误的英文标记改为跟随应用内中英文设置，并通过 HTML `translate=no` 与浏览器禁止机翻标记阻止 Chrome 对已本地化界面二次翻译；修复“周K→周克”“自选股超级看板→自选股看超级板”“筹码获利比例”等财经术语被浏览器篡改的问题。
- [修复] 首页自选股 K 线切换改为预热全部可见标的并复用 10 秒本地缓存；SQLite 行情请求由 120 秒长等待缩短为分时 20 秒/历史 45 秒，临时失败后自动退避重试，任何后续成功都会清除错误，有最近成功数据时继续展示而不再被“数据加载失败”永久盖住。
- [修复] 通用详情抽屉改为挂载到 `document.body` 顶层，并约束视口高度与内部滚动；小作文原文优先按知识星球永久 `topic_id` 定位，事件 ID 仅作回退，修复在长页面点击后抽屉跑到视口上方只剩黑色遮罩，以及事件重建后可能打开错误原文的问题。
- [新功能] 自选股“一致预期”改为独立专项分析：将每只股票最近 20 篇关键词匹配小作文重新交给 DeepSeek，按原文证据提取利润、EPS、目标价、目标市值、估值倍数等推测并持久化；每项区分上市公司、合并口径、子公司、收购标的和业务分部，过滤同行噪声，无明确预期时保持空结果，且不与 Tushare 券商一致预期混算。
- [改进] 一致预期界面升级为全宽研究工作台，隐藏无关行情与回填侧栏，新增券商预测矩阵、小作文输入/完成度、利润/目标价/市值锚点、证据卡、分歧与限制，并支持页内重跑和局部进度反馈。
- [新功能] 新增东方财富股吧真实公开股评源：按动态自选股每 2 分钟读取最新公开帖子页，增量保存作者、发布时间、正文摘录、浏览/回复/点赞、图片和原文链接；与千股千评结构化指标分开，并全量标记为待核验观点。
- [新功能] 投资情报台新增“全渠道数据源 BI”：展示全部已接入来源的存量、近 30 日事实趋势、最新事实时间、上轮收到/新增/更新、同步耗时、轮询频率、底层接口、SQLite 落库位置和可直接调用的事件 API。
- [修复] 来源健康不再把空返回当成有数据：同步运行状态与事实新鲜度分离，新增逐源抓取遥测；RSS/Atom/NewsNow 没有启用上游时明确标记“未配置”，配置后由监控轮询先现场抓取再进入统一事件库。
- [改进] 巨潮公告后台轮询由自选股范围升级为最近两日全市场分页增量抓取，15 分钟一次，仅入库公告元数据和官方原文链接，不自动下载 PDF 文件。
- [修复] 全站路由新增统一首屏数据就绪门：进入页面时以真实接口完成数显示进度条，核心请求全部结束后一次性展示页面；页内筛选、切换和后台刷新保留现有内容，不再反复遮挡，首次组件异常改为静默自动恢复而非闪出失败面板。
- [新功能] 自选股超级看板新增小作文简称关键词索引、站内原文窗口（正文/图片/附件）、券商与小作文分栏一致预期、相关新闻/知识星球/天眼查消息渠道及东方财富公开股评监控；新增自选股自动复用同一增量入库规则。
- [改进] 投资情报台收敛为全渠道信息中心：来源矩阵提升为首页并支持逐渠道查看真实消息，实时流水拆为独立页面，二级导航移除超级关注、市场结构、公司机构和综合研判入口。
- [改进] 小作文信息流新增全库资产看板，展示已入库总数、最早/最新原文日期、最近同步、知识星球数量、AI 覆盖率与状态分布，以及近 24 小时/7日/30日新增量。
- [修复] 修复补分析时 worker 已运行导致状态锁重复获取、接口永久等待并被前端误报为“上游服务超时”的死锁；历史入队请求改用独立三分钟等待预算。
- [新功能] 小作文信息流改为检索整个本地 SQLite 原文库，默认包含未入队和未完成 AI 分析的纪要；关键词覆盖标题、正文、作者、知识星球、股票代码及已有 AI 摘要/标签，并新增 AI 状态筛选。
- [修复] 前端热升级不再删除旧版内容哈希分包，已打开页面在升级后继续切换按需加载功能仍可正常访问；若资源确实缺失，则按构建版本穿透缓存并自动恢复，避免反复跳转“页面加载失败”。
- [新功能] 小作文数据管理新增按篇数补分析：可从本地未入队历史纪要中选择 50～5000 篇，并按最近或最早顺序加入 AI 队列；不会重新抓取知识星球、重复分析已有任务或隐式追加近 30 天任务。
- [调整] 取消知识星球“持续翻到整个星球最早主题”的全量历史回溯，仅保留近 1 年或近 2 年的可控幂等同步；旧纪要默认不进入 AI 队列，已入库记录不删除。
- [改进] 日内行情图恢复分钟级展示：已结束区间只返回一分钟 OHLCV，当前分钟由最新秒快照实时合成；横轴显示交易时间，纵轴按可见阶段最高/最低涨跌幅自适应，不再强制围绕昨收或 0% 对称留白。
- [修复] 全站前端稳定性巡检修复 SQLite 大批量参数溢出、行情历史批写失败及开机回补抢占首屏；历史维护延后并复用新上市标的的鲜活缓存，秒级图表和实时报价阻止请求重叠且后台标签页暂停轮询，小作文、量化、情报台和一站式取数在单渠道失败时保留其余成功事实数据，部署后旧分包缓存可自动恢复一次。
- [改进] 首页大盘主 K 线支持上证指数、上证50、沪深300、科创50、中证500、中证1000、深证成指和创业板指一键切换；市场广度改为仅接受上海当日全A股快照，并新增涨跌家数直方图、行业板块涨跌分布及领涨/领跌行业可视化，旧交易日数据不再回退展示。
- [改进] 小作文量化新增后台机构排名预计算：知识星球增量/历史回填后自动触发，完整事件集不再被行情补取上限截断；未做 AI 的旧纪要通过原文代码和本地股票名称索引进入回测，页面同步展示扫描覆盖、定价覆盖、成熟主榜和后台状态。
- [修复] 小作文全量机构榜改由独立进程预计算，避免解析数万篇原文时抢占 Web API；启动先快速产出本地行情榜，再按每批60只补齐缺失 Tushare 日线。
- [修复] 小作文量化事件增加本地证券主数据白名单，过滤日期、金额和订单号误识别出的六位伪股票代码，同时保留公司名称确定性匹配出的真实标的。
- [修复] 量化页首次加载以当前回测快照规则为准，不再拿最近保存的另一条规则覆盖表单，避免“机构全榜指标”与“单机构策略定义”错位显示。
- [修复] 后台全机构基线使用独立查询接口，不再被用户单机构回测覆盖；服务重启复用六小时内基线，避免无数据变化时重复全量扫描。
- [修复] 量化回测页补上统一暗色工作台容器并重排机构榜列宽、序号、滚动表头和后台状态，修复白色孤立界面与表格列未对齐的问题。
- [修复] 市场总览仅在报价日期属于上海当日时展示“当日涨跌幅”；Tushare 上一交易日回退值明确标记为上一收盘并隐藏涨跌幅，禁止旧收盘冒充当日行情。
- [改进] 首页重构为高密度工业终端：大盘分时图扩展为整页主视觉，自选股改为标的切换式大图；秒快照、上一交易日市场广度、北向资金、海外最近收盘和情报渠道分别显示真实来源与时间，修复盘中报价和前一日 K 线并列造成的口径错配。
- [修复] 日内行情合并改为分钟历史补缺、真实秒线覆盖同分钟，避免采到少量秒快照后整日分钟曲线被丢弃导致首页行情图稀疏。
- [改进] 小作文雷达拆分为今日研判、信息流、趋势追踪、每日报告和数据管理五个独立页面；首页优先展示需核验信号，详情先呈现原文，筛选、趋势与同步运维不再堆叠在同一长页面。
- [修复] 桌面端左侧导航栏约束内部最小宽度与菜单最大宽度，并扩展到可容纳完整中文菜单的宽度，修复文字固有宽度撑开侧栏、使激活背景突出外框或被迫省略的问题。
- [新功能] 个股与大盘秒级行情新增本秒成交量/成交额事实字段，由相邻累计快照安全差分生成；数据库同时保留当日累计口径，日内成交量副图改用真实秒增量，历史秒线自动回填。
- [改进] SQLite 事实库新增行情、知识、情报、分析和持仓逻辑域健康/新鲜度 API，启用 WAL 连接参数、每小时安全查询规划维护和被动检查点，并移除被唯一/复合索引完整覆盖的热表冗余索引。
- [改进] 投资情报到期渠道改为受控并发抓取、单线程串行入库，慢接口不再阻塞快讯；公司类接口自动跳过可转债和基金代码，来源状态补充刷新模式与目标周期。
- [新功能] 自选股行情升级为1秒固定节拍批量采集，新增去重的 `stock_ticks` 事实表；前端每秒读取本地行情，API降采样显示但SQLite保留完整秒级数据。
- [改进] 统一行情窗口放大并升级为分时/K线双模式，新增昨收基准、涨跌额、涨跌幅、OHLC、MA5/10/20 和成交量副图。
- [新功能] 首页大盘指数与自选股超级看板新增统一日内/日线/周线/月线/年线及年限切换；个股与指数行情分别落入本地 SQLite，周/月/年线从本地日线聚合，盘中后台按自选股持续采集分钟行情。
- [新功能] 自选股超级看板成为独立一级页面，新增自选股时自动创建最近半年全渠道回填任务，并逐渠道展示真实入库进度与失败状态。
- [新功能] 自选股超级看板支持逐只删除与二次确认；删除后立即停止该标的后续行情监控，同时保留已入库的历史行情、公告和研究资料。
- [改进] 投资情报台看板缩小字体和垂直间距，市场与公司看板默认只展示 10 条高价值事实，完整信息流移至事实流水页。

<!-- 新条目格式：- [类型] 描述（类型取值：新功能/改进/修复/文档/测试/chore）-->
<!-- 每条独立一行追加到本段末尾，无需分类标题，合并时冲突最小 -->

- [改进] 数据一站式获取改为持久化后台任务：按真实渠道完成数、文件导出、附件处理、ZIP 压缩和最终校验展示进度，切页后自动续看；数据包下载按实际传输字节展示百分比或已接收大小，不再使用“下载中”占位文案。

- [改进] 投资情报台升级为持续高频监控：Tushare 六路快讯与可配置媒体流 15 秒、知识星球 MCP 10 秒、股吧 30 秒、巨潮公告及市场技术 60 秒，其余机构/公司/资金/研报/天眼查均压缩至 2～10 分钟；默认并发提升至 16，快源抓取完成后立即增量入库，不再等待慢源结束；快讯使用 10 分钟重叠游标并在停机后自动扩窗补漏，实时流水、渠道首页和 BI 均每 10 秒读取本地增量。
- [修复] 数据源状态拆分“最近检查”和“最新事实”：上游没有新公告或研报时明确显示“实时巡检·上游静默”，不再用旧事实发布时间误导为后台没有更新；新增监控延迟、下一次检查、巡检 SLA 与快源优先入库回归测试。
- [修复] 修正 Tushare 快讯增量游标的 UTC/上海时区换算，避免 10 分钟实时窗口被错误扩成数小时并反复处理大量历史重复记录。
- [改进] 知识星球统一情报镜像改为按纪要同步时间和 AI 分析更新时间增量投影，保留 60 秒重叠去重，停止每 10 秒重复扫描近三日上千篇语料。

- [修复] 将盘中实时行情移出投资情报流和评分口径，行情状态改为每只关注股每日一条开盘快照与一条收盘日线，历史实时记录仅保留显式审计查询。
- [改进] 投资情报台完成五轮重塑并新增华懋科技、胜宏科技专属超级关注股工作台，完整利用行情估值、财务三表、技术、筹码资金、股权、公告、研报、天眼查和知识星球数据，提供双股同口径比较、决策快照去重与带事件 ID 的大模型证据包。

- [修复] 修复 Web 首页个股栏在 stock-bar 摘要字段缺失或动作建议无法归类时隐藏情绪分与建议标识的问题。
- [新功能] 新增统一财经数据 API，支持 Tushare Pro 全 api_name 按需调用及知识星球 MCP 调研纪要的幂等同步、检索与详情查询。
- [新功能] 新增小作文雷达：DeepSeek V4 对近 30 天知识星球调研纪要持续打标签并提取股票观点、催化剂和风险，提供可恢复队列、实时 MCP 入库联动、看板与筛选 API。
- [新功能] 新增投资情报台：统一 Tushare 行情/快讯/公司/机构数据、知识星球小作文和 RSS 情报，提供自选股三视角评分、实时轮询及外部 API/MCP 幂等接入。
- [测试] 补充投资情报源归一化、股票关联、幂等写入和三视角评分回归测试。
- [改进] DeepSeek V4 Flash 小作文 worker 显式并发上限放宽至 2000，默认值保持保守并继续通过环境变量控制。
- [新功能] 新增 iCloud 云端知识数据库：定时导出知识专用 SQLite 版本到 iCloud Drive，提供敏感表排除、SHA-256、完整性校验、版本保留、状态 API 和 Web 控制入口。
- [新功能] 投资情报台集成巨潮资讯上市公司公告，支持日期、个股、26 类公告和关键词抓取、幂等入库、统一公司视角监控及 PDF 原文访问。
- [测试] 补充巨潮证券 orgId 解析、公告归一化、分类校验、HTTPS PDF 与幂等同步回归测试。
- [改进] 将原上市公司报表 GUI 的查询、Excel 导出、PDF 下载和文本提取完整改写为统一公告库 API，支持安全缓存及 PDF+TXT+索引打包。
- [修复] 小作文实时链路改为应用直接调用知识星球 MCP 的游标增量同步，不再仅轮询本地 SQLite，并新增每星球同步状态、立即同步与 worker 控制 API。
- [新功能] 知识星球纪要支持图片和文件安全落盘、SQLite 元数据索引、历史附件按需补取及小作文详情展示，新纪要入库后立即进入 DeepSeek 队列和统一监控流。
- [新功能] 小作文深度分析升级到 v2，新增原文证据、信源质量、信息增量、矛盾、证伪条件、盈利/估值影响及后续验证清单，并按提示词版本自动重算。
- [新功能] 新增按上海自然日生成的多模型小作文日报，以及股票、标签、主题的日/周/月词云和前周期变化。
- [改进] 小作文分析升级为证据驾驶舱：新增 14 日情绪/重要度趋势、证据质量矩阵、关注股日周月专项信号、高信息增量列表和仅基于已完成报告的模型共识；日报 v2 对前一日全量数据聚合并抽取 240 条代表性证据深度归纳。
- [新功能] 新增 macOS 登录自启动安装脚本与“财经情报台.app”双击入口，后台服务直接复用当前项目的 `.env` 和 SQLite，并仅监听本机回环地址。
- [改进] 将 Tushare 15000 积分数据从通用转发升级为产品化数据域：财务三表/指标/预告、筹码分布、融资融券、北向持股、技术因子、质押/解禁/增减持/回购、完整券商 PDF 研报、长篇新闻和新闻联播均进入统一事件库与渠道分流。
- [改进] 自选股监控改为华懋科技与胜宏科技；行情 5 分钟、技术 15 分钟、资金 1 小时、财务与股权 6 小时，避免每分钟重复抓取。
- [新功能] 新增“数据一站式获取”：DeepSeek 将自然语言需求拆解为可审计的跨渠道任务，统一调用 Tushare、知识星球、公告/情报台和天眼查，并打包交付 JSON、CSV、Excel、ZIP 与来源清单。
- [修复] 一站式取数新增强制 scope 契约与返回后二次过滤，禁止非全市场需求混入无关新闻或情报；导出改为按渠道分目录并分别生成 Excel。
- [改进] Web 首页重构为市场总览大看板，聚合 A 股核心指数、全市场涨跌分布、北向资金、六大海外指数、自选股超级看板、最新情报与 AI 快捷入口，并采用五分钟缓存。
- [改进] 投资情报台升级为事实优先的渠道终端，新增六路财经快讯、龙虎榜/机构席位/大宗交易、公司治理与股东、天眼查五维企业事实等渠道，并为每条事件记录证据等级、原始 API、数据时间、入库时间与原文可用性；知识星球观点默认与事实流隔离。
- [改进] 统一财经数据接口新增巨潮公告、天眼查企业事件和统一监控事件查询，并完整暴露23个后台数据渠道的新鲜度、轮询周期、错误与累计记录，避免已接入数据源在接口中不可见。
- [改进] Tushare 监控新增同花顺概念/行业资金流与涨停池、炸板池、跌停池事实快照，并补齐管理层名录、市场两融总量和神奇九转；集合竞价接口因当前凭证缺少独立权限而明确暂不接入。
- [测试] 补充题材资金流排序、涨跌停池计数、事实中性语义及 Tushare 原始接口追踪回归测试。
- [改进] 小作文日报升级为 v3 分层证据选择，优先保留华懋科技、胜宏科技证据并覆盖不同来源、类别和信息性质；跨模型共识/分歧改为比较结构化主题方向与个股立场。
- [测试] 补充低频关注股不被热门主题挤出以及跨模型同义共识、个股方向分歧识别回归测试。
- [修复] 一站式取数执行器将公告任务的 Tushare 风格 `ts_code`/紧凑日期归一化为公告库参数并过滤无效字段，避免模型计划正确但执行阶段失败。
- [改进] 一站式取数规划器强化“只取明确要求维度”，并开放概念/行业资金流、涨跌停池、市场两融、神奇九转和管理层名录等新增 Tushare 能力。
- [修复] 一站式取数范围层将已配置关注股名称解析为证券代码，避免模型只返回公司名时公告与结构化数据查询漏数。
- [修复] 小作文雷达将概览、纪要列表和运行状态轮询解耦，筛选不再重复触发全量聚合，单个慢接口也不会让整页真实数据误显示为零。
- [修复] macOS 后台 LaunchAgent 加入用户级命令目录，使已安装授权的天眼查 `tyc` CLI 在开机自启服务中可被识别和调用。
- [改进] 知识星球同步改为信息量增量：点赞、阅读、评论等互动数字不再刷新数据库或触发 AI；标题、正文、图片和文件变化才更新，并用批量哈希比对实现旧帖零写入、零排队。
- [修复] iCloud 知识库版本列表不再逐个拉取云端 manifest，占位文件环境下状态接口可即时返回真实版本数，同秒版本按纳秒时间正确保留。
- [修复] A 股日线缓存统一使用六位证券代码存储和查询，避免 `.SH`/`.SZ` 后缀造成历史行情分裂与回测缺口。
- [改进] 事实流水拆分总览、事件、公告和云状态请求；筛选仅刷新事件列表，后台轮询不再重复读取 iCloud 与公告目录。
- [改进] 数据源状态拆分为“同步成功”和“事实新鲜度”两套口径，逐源显示最新事实时间、入库时间、存量记录及新鲜/陈旧/空源状态，避免空返回被显示为健康数据。
- [改进] 首页十个海内外指数及市场摘要改为受控并发获取；首页、情报总览和超级关注股列表仅传展示所需证据字段，完整结构化数据继续由事件详情按需读取。
- [修复] 小作文每日报告 worker 状态接口不再在前端轮询时重复返回完整报告正文，仅保留日期、样本数和各模型执行状态。
- [修复] 清除旧版盘中实时行情遗留记录，投资情报库只保留每日开盘快照与收盘日线；清理前生成独立 SQLite 可恢复备份。
- [修复] 首页自选股行情卡直接使用最新 Tushare 日线补齐收盘价、涨跌幅、开高低与成交额，避免已有日线却显示为空。
- [修复] 投资情报总览图表容器补齐最小宽度约束，消除首次布局产生的 Recharts 负尺寸警告。
- [新功能] 新增“量化回测与数据利用”页面，将知识星球小作文结构化观点与 Tushare 日线、复权因子和沪深300基准连接，支持自定义券商/研究组规则、5/10/20日事件研究、近30日首次提及、吹票强度分组、趋势买点和等权组合。
- [新功能] 新增可持久化的小作文量化规则与不可变回测快照，记录规则参数、数据截止日、事件源哈希、成熟样本和完整结果，原始事件可直达知识星球原文。
- [改进] 研究组胜率排名同时展示原始胜率和中性先验收缩胜率，避免单一样本100%胜率误导排名；未走满持有期的事件不进入胜率。
- [文档] 新增小作文量化事件日、买卖价、超额收益、首次提及、复权、吹票强度、组合和 API 口径说明。
- [重构] 数据一站式获取升级为 `channel-scoped-v3-live`：公告改为巨潮官方接口现场检索，Tushare 新闻、可下载研报和盈利预测按独立 `api_name` 现场调用并分页，统一情报缓存不再代替官方数据源。
- [新功能] 天眼查增加 L0 企业锚定和 24 维企业全景查询；自然语言要求文件时自动下载巨潮 PDF、Tushare 研报、知识星球图片附件及结果内 PDF，写入来源清单并随 ZIP 交付。
