...
财经情报台是一套面向个人投资研究的本地化财经数据、情报监控与分析平台。
...
系统默认运行在本机，SQLite 保存结构化数据；账号凭据、API Token、运行数据库、缓存和日志不会进入 Git 仓库。
...
| 小作文雷达 | 从知识星球 MCP 增量同步原文，进行实时队列分析、日报、词云和个股提及跟踪 |
...
小作文雷达还可后台回填近 1 年或近 2 年历史纪要。历史内容默认只入库供检索、首次提及研究和量化回测，不自动消耗 AI 额度；用户明确点击“按需 AI 分析”后才进入模型队列。未分析历史中含有明确证券代码的原文，可用确定性关键词方向作为基础回测事件，并始终标记为 `raw_unanalyzed`。
