# -*- coding: utf-8 -*-
"""End-to-end coverage for approval, trusted-IP login and stable identities."""
...
def test_approval_enables_ip_login_and_session_keeps_identity_stable(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert status.json()["authMethod"] == "trusted_ip"
        assert "dsa_user_session" in applicant.cookies
        ...
        # The first browser already owns a signed session, so a later account
        # on the same NAT cannot silently change its identity.
        assert applicant.get("/api/v1/private", headers=first_ip).json()["userId"] == first_id
