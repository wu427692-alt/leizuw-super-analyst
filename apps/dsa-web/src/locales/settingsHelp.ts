...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_ENABLED': {
    title: '低成本模型日历轮换',
    summary: '按上海日期在 DeepSeek V4 Flash 与 Kimi Code 之间自动切换主通道。',
    usage: '启用后，默认每月 9—23 日使用 Kimi Code，24 日至次月 8 日使用 DeepSeek V4 Flash。',
    impact: ['影响机构段子分析、日报、录音纪要、一站式取数、普通分析和未单独指定模型的问股。'],
    notes: ['主通道失败时只回退另一个低成本通道；切换日期无需重启服务。'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_TIMEZONE': {
    title: '模型轮换时区',
    summary: '决定日历轮换在哪个时区的午夜切换。',
    usage: '默认 Asia/Shanghai；建议云端与本地保持一致。',
    notes: ['必须使用有效的 IANA 时区名称。'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_KIMI_START_DAY': {
    title: 'Kimi 起始日',
    summary: 'Kimi Code 成为主通道的每月起始日期（含当日）。',
    usage: '默认 9，允许 1—31。',
    impact: ['从该日开始，到 DeepSeek 起始日前一天使用 Kimi Code。'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_DEEPSEEK_START_DAY': {
    title: 'DeepSeek 起始日',
    summary: 'DeepSeek V4 Flash 成为主通道的每月起始日期（含当日）。',
    usage: '默认 24，允许 1—31。',
    impact: ['从该日开始，到下月 Kimi 起始日前一天使用 DeepSeek。'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_KIMI_MODEL': {
    title: 'Kimi 轮换模型',
    summary: '指定 Kimi 日期窗口使用的已配置低成本模型。',
    usage: '默认 openai/kimi-for-coding，必须能在 kimi_code 渠道中找到。',
    notes: ['Kimi Code 路由会保留 thinking=disabled，并移除官方不允许强制覆盖的采样参数。'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_DEEPSEEK_MODEL': {
    title: 'DeepSeek 轮换模型',
    summary: '指定 DeepSeek 日期窗口使用的低成本模型。',
    usage: '默认 deepseek/deepseek-v4-flash，必须能在 deepseek 渠道中找到。',
    notes: ['生产配置仅保留 V4 Flash，不会自动选择 Pro。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_ENABLED': {
    title: 'Low-Cost Calendar Rotation',
    summary: 'Automatically rotates the primary channel between DeepSeek V4 Flash and Kimi Code by Shanghai calendar date.',
    usage: 'By default, Kimi Code is primary on days 9–23 and DeepSeek V4 Flash is primary from day 24 through day 8 of the next month.',
    impact: ['Affects essay analysis, daily reports, audio minutes, data acquisition, regular analysis, and Agent calls without an explicit model.'],
    notes: ['Only the other low-cost channel is used as fallback; date changes do not require a service restart.'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_TIMEZONE': {
    title: 'Provider Rotation Timezone',
    summary: 'Sets the timezone whose midnight boundary controls provider rotation.',
    usage: 'Default is Asia/Shanghai; keep local and cloud environments aligned.',
    notes: ['Use a valid IANA timezone name.'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_KIMI_START_DAY': {
    title: 'Kimi Start Day',
    summary: 'The inclusive calendar day on which Kimi Code becomes primary.',
    usage: 'Default is 9; accepted values are 1–31.',
    impact: ['Kimi remains primary until the day before the DeepSeek start day.'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_DEEPSEEK_START_DAY': {
    title: 'DeepSeek Start Day',
    summary: 'The inclusive calendar day on which DeepSeek V4 Flash becomes primary.',
    usage: 'Default is 24; accepted values are 1–31.',
    impact: ['DeepSeek remains primary until the day before the next Kimi start day.'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_KIMI_MODEL': {
    title: 'Kimi Rotation Model',
    summary: 'Selects the configured low-cost model used during the Kimi calendar window.',
    usage: 'Defaults to openai/kimi-for-coding and must exist in the kimi_code channel.',
    notes: ['The Kimi Code route keeps thinking disabled and removes sampling overrides prohibited by the official interface.'],
  },
  'settings.ai_model.LLM_PROVIDER_SCHEDULE_DEEPSEEK_MODEL': {
    title: 'DeepSeek Rotation Model',
    summary: 'Selects the low-cost model used during the DeepSeek calendar window.',
    usage: 'Defaults to deepseek/deepseek-v4-flash and must exist in the deepseek channel.',
    notes: ['The production configuration contains V4 Flash only and does not automatically select Pro.'],
  },
  'settings.ai_model.LITELLM_CONFIG': {
