...
const legacyChapters: GuideChapter[] = [
  {
...
const chapters: GuideChapter[] = [
  ...legacyChapters.filter((chapter) => !['research-center', 'monitor'].includes(chapter.id)),
  {
    id: 'tasks', category: '数据量化', eyebrow: '所有后台工作统一查看', title: '任务与验证', icon: ListChecks,
    summary: '集中查看行业与公司调研、量化研究、录音转写、录音打包和一站式取数任务。',
    href: '/tasks', action: '进入任务与验证',
    purpose: '让长任务离开原页面仍可运行，并按当前用户保存进度、结果和失败原因。',
    data: ['行业与公司调研任务', '量化任务', '录音转写与纪要任务', '录音文件打包任务', '一站式取数任务'],
    outputs: ['统一任务状态', '真实执行进度', '完成结果入口', '失败与重试入口'],
    steps: ['从对应研究工具发起任务。', '进入任务与验证查看全部后台工作。', '按执行中、已完成或失败筛选。', '点击任务返回对应结果页继续处理。'],
    modules: [
      { name: '任务总账', route: '/tasks', purpose: '汇总五类后台任务。', data: '按用户隔离的任务状态库', actions: ['筛选任务', '查看进度', '打开结果'], output: '统一任务与结果入口。' },
      { name: '新建入口', route: '/tasks', purpose: '快速进入调研、量化、录音或取数。', data: '各工作台任务能力', actions: ['选择任务类型', '进入对应工作台', '提交任务'], output: '新的后台任务。' },
    ],
    verify: ['离开原页面后任务必须继续执行。', '用户只能看到自己的任务。', '失败任务必须保留原因，不能伪装完成。'],
  },
];
...
const quickTasks = [
  ...
  { title: '查看后台任务进度', detail: '调研 → 量化 → 转写 → 取数 → 结果', href: '#tasks', icon: ListChecks },
...
const workflowRows = [
  { title: '公司首次建档', pages: '自选股超级看板 → 深度研判 → 问股', process: '加入股票并补齐半年资料；读事实、财务和反证；再带上下文追问。', finish: '得到公司证据包、缺口和持续跟踪入口。' },
  { title: '重大事件核验', pages: '首页新闻 → 自选股 → 数据一站式获取', process: '发现重要新闻；对照行情与预期；下载官方原文和相关研报。', finish: '形成事件时间线、市场反应和原始证据包。' },
...
const faqs = [
  ...
  ['管理员数据源页里的“收到”和“新增”有什么区别？', '“收到”是上游本轮返回条数；“新增”是去重后首次进入本地库的条数；“更新”是已有记录发生结构化更新。三者不能相加当作新情报。'],
