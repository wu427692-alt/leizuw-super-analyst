...
function OverseasCard({ item }: { item: MarketIndexCard }) {
  ...
  const verifiedChangePct = session.canShowChange ? item.changePct : null;
  return <div className="grid min-w-0 grid-cols-[96px_minmax(0,1fr)_72px] items-center gap-3 border-b border-border/55 px-3 py-2 last:border-b-0">
  ...
  return <AppPage className="market-command-center max-w-none px-2 pb-6 pt-2 md:px-3 lg:px-4">
    <div className="space-y-2">
      <section className="market-panel overflow-hidden border border-border/70 bg-card/90">
        <div className="flex flex-col border-b border-border/60 px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex overflow-x-auto">{cnIndices.map((item) => <IndexTicker key={item.code} item={item} selected={item.code === leadIndex?.code} onSelect={() => setSelectedIndexCode(item.code)} />)}
