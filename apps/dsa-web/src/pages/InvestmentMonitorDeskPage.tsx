...
function WatchlistView() {
  ...
  useEffect(() => { Promise.all(['603306.SH', '300476.SZ'].map(code => investmentMonitorApi.symbol(code, 30))).then(setItems).finally(() => setLoading(false)); }, []);
...
function DomainView({ mode }: { mode: 'market' | 'company' }) {
  ...
  useEffect(() => { investmentMonitorApi.events({ days: 14, channel: config.channels, pageSize: 40 }).then(result => setEvents(result.items)).finally(() => setLoading(false)); }, [config.channels]);
...
function AnalysisView() {
  ...
  useEffect(()=>{ investmentMonitorApi.intelligenceDashboard(14).then(setData); },[]);
...
export default function InvestmentMonitorDeskPage() {
  ...
  return <AppPage className="max-w-[1680px]"><div className="overflow-hidden border border-[#C9CCD2] bg-[#F7F7F8] text-[#17181A] shadow-[0_18px_50px_rgba(17,24,39,0.08)]">{mode==='watchlist'?<WatchlistView/>:mode==='market'?<DomainView mode="market"/>:mode==='company'?<DomainView mode="company"/>:<AnalysisView/>}</div></AppPage>;
}
